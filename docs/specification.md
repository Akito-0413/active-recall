...
├─ Actions: 認証、手動フォーム、プロフィール更新向けServer Actions
...
- `features/auth/` と `lib/auth/` に認証関連処理を集約する。
...
| カラム | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| id | uuid | yes | `auth.users.id` と一致する主キー |
| email | text | yes | ログインメールアドレス |
| username | text | yes | 表示名。初期値はメールアドレスの `@` より前 |
...
| --- | --- | --- | --- |
| id | uuid | yes | 主キー |
| owner_id | uuid | no | 所有者ユーザーID。画面作成ログでは必須、外部GPT保存では当面nullを許可 |
...
- Google OAuthの認可コードをSupabaseセッションへ交換する。

認証:

- Supabase AuthのPKCEフローを使用する。

処理:

- クエリの `code` を受け取る。
- `exchangeCodeForSession` を実行する。
- 成功時は `/logs` へ戻す。
- `code` 不足や交換失敗時は `/login` へ戻す。
...
- 外部保存ログは当面 `owner_id = null` とし、ログイン画面からは表示しない。
...
- `password` は必須で、8文字以上。
- `username` は必須で、空白のみ不可、適切な最大長を設ける。
- 初期 `username` はメールアドレスの `@` より前を使う。
...
- ログイン済みなら `/logs`、未ログインなら `/login` へ導く。
...
- メール+パスワードまたはGoogleでアカウントを作成する。

### `/account`

目的:

- ユーザーID、メールアドレス、ユーザー名を確認し、ユーザー名を更新する。
...
- Supabase Authは `@supabase/ssr` を使ったCookieベースSSR構成を前提にする。
- 画面系の学習ログ操作はRLSで本人データのみに制限する。
