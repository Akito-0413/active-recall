...
- Supabase AuthとRLSで、自分の学習ログだけを扱える構造にする。
...
- Feature UI層: 学習ログ一覧、詳細、手動登録、復習画面、認証画面、アカウント画面。
...
MVPでは認証ありを前提に、ユーザー所有をDBとアプリ側の両方で扱います。

- Supabase AuthのCookieセッションを使う。
- `recall_logs.owner_id` で学習ログの所有者を表現する。
- `user_profiles` と `recall_logs` にRLSを適用する。
- Data Access層では認証済みserver clientを使い、service roleへの依存を最小化する。
...
- Supabase Authを使用する。
- メール+パスワードログインとGoogleログインを提供する。
- `proxy.ts` でセッション更新と保護ページのリダイレクトを行う。
- Server Components、Server Actions、Route Handlersでは認証済みユーザーを都度確認する。
- 学習ログとプロフィールはRLSで本人だけがアクセスできるようにする。
...
認証、AI連携、復習スケジュールの影響範囲に応じてテスト範囲を広げます。
